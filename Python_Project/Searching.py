...
def StartingScreen():
    ...
    def search():
        ...
        if (checkin >= checkout):
            frame5 = Frame(root)
            frame5.pack(fill=X)
            Label(frame5, text="Invalid Dates", width=11, font=('Arial', '12'),fg='red').pack()
        else:
            root.destroy()
...
def printOwnerHotels(root,arr,ro) :
    ...
    def delete():
        root.destroy()
        que.qdeleteHotel(arr[-1],gid)
        que.qshowOwnerHotels(gid)
    Label(root, text=str(arr[0]), font=('Arial', '12'), width=10, fg='OrangeRed3').grid(row=ro, column=0)
    for i in range(1, len(arr)-1):
        Label(root, text=str(arr[i]), font=('Arial', '12'), width=13).grid(row=ro, column=i)
    plen = que.qHotelPlen(arr[-1])
    Label(root, text=str(plen) + '%', font=('Arial', '12'), width=10).grid(row=ro, column=i+1)
    Button(root, text='Manage Rooms', font=('Arial', '12'), width=14,bg='gray25',fg='white',command=ManRoom).grid(row=ro, column=i+2)
    Button(root, text='Delete Hotel', font=('Arial', '12'), width=14, bg='brown3', fg='white', command=delete).grid(row=ro, column=i + 3)
...
def addRoom(check):
    ...
    def add():
        ...
        def fill():
            ...
            check = que.qcheckHotelHas(hid,idRoom)
            if (check == 1):
