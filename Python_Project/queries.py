...
def adduser(username, password, fname, lname, email, idnum, pnum, country, sex, addr, birth):
    ...
    tempuse = "\'" + tempuse[0] + "\'"
    if tempuse == username: return 1 #invalid username
    password = "\'" + password + "\'"
    if len(password) < 6  and len(password) > 25: return 0   #failed registration
    fname = "\'" + fname + "\'"
...
def qdeleteHotel(idHot,idUser):
    ida = str(tuple(idUser))
    ida = ida.replace(')', '')
    ida = ida.replace(',', '')
    ida = ida.replace('(', '')
    # GET ONLY THE NUMBER FROM THE ID
    ...
    mydb = mysqlc.connect(host="150.140.186.221", user="db20_up1046975", password="up1046975",database='project_db20_up1046975')
    mycursor = mydb.cursor()
    strn = " select count(*) from MAKE " \
           " join GUEST on id_guest = m_id_guest join RESERVATION on id_reservation = m_id_reservation " \
           " where r_id_hotel = " + str(idHot) + " and id_guest = " + str(ida) + " group by (m_id_guest)"
    mycursor.execute(strn)
    x = mycursor.fetchall()
    xa = str(tuple(x))
    xa = xa.replace(')', '')
    ...
    xa = xa.replace('(', '')
    if x!=[]:
        strn = " update GUEST set NOreservations = NOreservations - " + str(xa) + " where id_guest = " + str(ida)
        mycursor.execute(strn)
