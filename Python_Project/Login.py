...
def reg():
    ...
    def adduser():
        ...
        if (y == 0):
            Label(root, text="Registration Failed", font=('Arial', '12'), fg='red').grid(row=13, column=1)
        elif (y == 1):
            Label(root, text="Invalid Username", font=('Arial', '12'), fg='red').grid(row=12, column=1)
        else:
            root.destroy()
            log()
...
def log():
    ...
    root.resizable(FALSE, FALSE)
    root.iconbitmap('dbic.ico')
    Label(root, text="Username:", font=('Arial', '12')).place(x=25, y=15)
    e1 = Entry(root)
    e1.place(x=120, y=18)
    Label(root, text="Password:", font=('Arial', '12')).place(x=25, y=55)
    e2 = Entry(root, show="*")
    e2.place(x=120, y=58)
